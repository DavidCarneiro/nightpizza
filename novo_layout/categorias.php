...
<section id="blg_sec">
	<div class="container">
		...
		<?php  
			...
			if($_GET[botao] == 'del'){
			$del = $mysqli->query("delete from dl_categorias where id = ".$_GET[id]."");   
		  }
		  if($_POST[cadastrar]){
			  if(!$_POST[cat]){
				echo "<script>alert('Por favor preencha o campo categoria!');</script>";  
			  }else{ 
			     $inserir = $mysqli->query("insert into dl_categorias (nome) values ('".ucwords($_POST[cat])."')");
			...
			?>
				...
				<div class="title_sec">
					...
					<h2>gerencie suas categorias</h2>
				</div>
				<section id="protfolio_sec">
					<div class="col-lg-12">
					  <div class="linkmenu">
						  <a href="admin.php"><i class="fa fa-home" aria-hidden="true"></i>Início</a>  &nbsp;&nbsp;|&nbsp;&nbsp;  
						  <a href="categorias.php" id="active"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>Categorias</a>  &nbsp;&nbsp;|&nbsp;&nbsp;  
					...
					<div class="col-lg-12" >
						<div id="cnt_form">
							<form id="contact-form" class="contact" method="post" action="categorias.php">
								...
								<input type="text" name="cat" class="form-control name-field" required="required" placeholder="Categoria">
								</div>
								<div class="form-group">
								...
								<div>
								  <h3>Categorias</h3>
								  <hr>
								  <?php $sel = $mysqli->query("select * from dl_categorias order by nome asc");
								  while($cat = $sel->fetch_array()){?>
								  <p><span><?=$cat[nome]?></span> | <span><a href="categorias.php?botao=del&id=<?=$cat[id]?>"><img src="img/x.png" width="15" height="15"></a></span></p>
								  <?php }?>
								</div>
							</form> 
						</div>				
